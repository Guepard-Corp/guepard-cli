...
use crate::application::services::{clone, compute, deploy, performance};
...
async fn create_clone(
    ...
    let repository_name = args
        .repository_name
        .clone()
        .unwrap_or_else(|| format!("clone-{}", deployment.repository_name));
    ...
    let username = clone_response
        .database_username
        .clone()
        .unwrap_or_else(|| "N/A".to_string());
    let password = clone_response
        .database_password
        .clone()
        .unwrap_or_else(|| "N/A".to_string());
    ...
    let connection_uri = format!(
        "postgresql://{}:{}@{}:{}/{}",
        username, password, clone_response.fqdn, port, clone_response.repository_name
    );
    ...
    if output_format == OutputFormat::Json {
        ...
        if let Some(compute_info) = &compute_data {
            ...
            if let Ok(branches) =
                crate::application::services::branch::list_branches(&clone_response.id, config)
                    .await
            {
                if let Some(branch) = branches.iter().find(|b| b.id == *attached_branch_id) {
                    ...
                    let (snapshot_name, snapshot_comment) = if let Ok(snapshots) =
                        crate::application::services::commit::list_all_commits(
                            &clone_response.id,
                            config,
                        )
                        .await
    ...
    println!(
        "  {} {}",
        "Provider:".yellow(),
        clone_response.database_provider
    );
    println!(
        "  {} {}",
        "Version:".yellow(),
        clone_response.database_version
    );
    ...
    match compute::list_compute(&clone_response.id, config).await {
        Ok(compute_info) => {
            ...
            // Get branch details
            match crate::application::services::branch::list_branches(&clone_response.id, config)
                .await
            {
                Ok(branches) => {
                    if let Some(branch) = branches.iter().find(|b| b.id == *attached_branch_id) {
                        ...
                        let snapshot_id = &branch.snapshot_id;
                        match crate::application::services::commit::list_all_commits(
                            &clone_response.id,
                            config,
                        )
                        .await
                        {
                            Ok(snapshots) => {
                                if let Some(snapshot) =
                                    snapshots.iter().find(|s| s.id == *snapshot_id)
                                {
                                    ...
                                    if !snapshot.snapshot_comment.is_empty() {
                                        println!(
                                            "  {} {}",
                                            "Comment:".yellow(),
                                            snapshot.snapshot_comment.cyan()
                                        );
                                    }
                                    println!(
                                        "  {} {}",
