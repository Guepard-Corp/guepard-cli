...
pub async fn deploy(args: &DeployArgs, config: &Config, output_format: OutputFormat) -> Result<()> {
    ...
    if let Some(deployment_id) = &args.deployment_id {
        ...
        if args.database_provider.is_some()
            && args.database_version.is_some()
            && args.region.is_some()
            && args.instance_type.is_some()
            && args.datacenter.is_some()
            && args.database_password.is_some()
        {
            ...
            if output_format == OutputFormat::Table {
                ...
                println!("  • Get/Update/Delete: -x <deployment_id> (and optionally -n for update, --purge for delete)");
                println!(
                    "{} Use 'guepard deploy --help' for more information",
                    "💡".yellow()
                );
            } else {
                print_json(&serde_json::json!({
                    "error": "Missing required arguments",
                    "usage": "Provide either creation arguments or a deployment ID for get/update/delete (use --purge to delete)"
                }));
            }
        }
    }
...
async fn create_deployment(
    ...
    let performance_profile_label = args
        .performance_profile
        .clone()
        .unwrap_or_else(|| "gp.g1.xsmall".to_string());
    ...
    let request = CreateDeploymentRequest {
        repository_name: args
            .repository_name
            .clone()
            .unwrap_or("default-repo".to_string()),
        database_provider,
        database_version,
        deployment_type: args
            .instance_type
            .clone()
    ...
    let deployment_type_display = if is_clone {
        "Clone".cyan()
    } else {
        deployment.deployment_type.clone().cyan()
    };
...
async fn get_deployment(
    ...
    if let Some(compute_info) = &compute_data {
        let attached_branch_id = compute_info
            .branch_id
            .as_ref()
        ...
        if let Ok(branches) = branch::list_branches(deployment_id, config).await {
            if let Some(branch) = branches.iter().find(|b| b.id == *attached_branch_id) {
                ...
                let (snapshot_name, snapshot_comment) =
    ...
    if is_clone {
        if let Some(deployment_parent) = &deployment.deployment_parent {
            println!(
                "  {} {}",
                "Deployment Parent:".yellow(),
                deployment_parent.cyan()
            );
        }
    }
    ...
    if is_clone {
        if let Some(snapshot_parent) = &deployment.snapshot_parent {
            println!(
                "  {} {}",
                "Snapshot Parent:".yellow(),
                snapshot_parent.cyan()
            );
        }
    }
...
async fn delete_deployment(
    ...
    let branches = branch::list_branches(deployment_id, config)
        .await
        .unwrap_or_default();
    ...
    if output_format == OutputFormat::Table {
        ...
        if purge_items.is_empty() {
            println!(
                "  {} {}",
                "Branches/datasets to purge:".dimmed(),
                "(none)".dimmed()
            );
        } else {
...
async fn interactive_deploy(config: &Config) -> Result<()> {
    ...
    let database_provider = if database_provider.is_empty() {
        "PostgreSQL"
    } else {
        database_provider
    };
    ...
    let database_version = if database_version.is_empty() {
        default_version
    } else {
        database_version
    };
    ...
    let datacenter = if datacenter.is_empty() {
        "us-west-aws"
    } else {
        datacenter
    };
    ...
    let performance_profile = if performance_profile.is_empty() {
        "gp.g1.xsmall"
    } else {
        performance_profile
    };
