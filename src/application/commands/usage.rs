use crate::application::output::{print_table_or_json, OutputFormat};
...
pub async fn usage(_args: &UsageArgs, config: &Config, output_format: OutputFormat) -> Result<()> {
    ...
    let rows = vec![
        UsageRow {
            ...
            quota: usage.quota_deployments,
            used: usage.usage_deployments,
        },
        UsageRow {
            resource: "Snapshots".to_string(),
            quota: usage.quota_snapshots,
            used: usage.usage_snapshots,
        },
        UsageRow {
            resource: "Clones".to_string(),
            quota: usage.quota_clones,
            used: usage.usage_clones,
        },
    ];
