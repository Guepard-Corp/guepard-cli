use crate::application::dto::compute_dto::{ListComputeResponse, StatusErrorResponse};
...
pub async fn get_status(deployment_id: &str, config: &Config) -> Result<(), ComputeError> {
    let jwt_token = config::load_jwt_token()
        .map_err(|e| ComputeError::SessionError(e.to_string()))?;
    let client = Client::new();
    let response = client
