...
pub enum SubCommand {
    ...
    ///
    ///   # Deploy with Tenet masking (proxy.yaml from your machine)
    ///   guepard deploy -p PostgreSQL -v 17 -r us-west-aws -i REPOSITORY -d us-west-aws \\
    ///     -n myapp -w secret --masked --proxy-config ./proxy.yaml
    Deploy(DeployArgs),
...
pub struct DeployArgs {
    ...
    /// Deploy with Tenet PII masking (requires --proxy-config)
    #[clap(long)]
    pub masked: bool,

    /// Local proxy.yaml for Tenet (required when --masked)
    #[arg(long, required_if_eq("masked", "true"))]
    pub proxy_config: Option<std::path::PathBuf>,

    /// Tenet masking salt (auto-generated by API if omitted)
    #[arg(long, requires = "masked")]
...
pub struct TenetDeployCliArgs {
    ...
    /// Address shown in hints / where clients connect to Tenet (VPC or node IP, e.g. 10.0.4.20), not necessarily the upstream DB host.
    #[arg(long)]
